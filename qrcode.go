...
func NewPositionDetectionPattern(PDPs [][]*PosGroup) (*PositionDetectionPatterns, error) {
	if len(PDPs) < 3 {
		fmt.Println("lost Position Detection Pattern")
...
func DecodeImg(img image.Image, batchPath string) (DecodeImageReturn, error) {
	matrix := new(Matrix)
	matrix.OrgImage = img
	...
	myBoundary := Boundary{XStart: leftStart.X, YStart:topStart.Y, XEnd:topEnd.X, YEnd:leftEnd.Y}

	matrix.Size = image.Rect(0, 0, len(matrix.Points), len(matrix.Points))
	myReturn := DecodeImageReturn{TheMatrix: matrix, TheBoundary: myBoundary}
	return myReturn, nil
}
...
func Decode(fi io.Reader) (*Matrix, error) {
	...
	qrDecodedImage, err := DecodeImg(img, batchPath)
	qrMatrix := qrDecodedImage.TheMatrix
	if err != nil {
		return nil, err
	}
	...
	qrMatrix.Boundary = qrDecodedImage.TheBoundary
	return qrMatrix, nil
}
